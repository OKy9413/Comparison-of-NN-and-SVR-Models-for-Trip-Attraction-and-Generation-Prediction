EvaluacionResultados <- function(actual, predicciones, Nombre, Escala, Tipo, hiper=FALSE, path) {
  # Cargar librerías
  if (!require(caret)) {
  ...
  if (!isFALSE(hiper)) {
    ...
    parameters <- data.frame(Nombre, round(RMSLE, 3), round(slope, 3), round(intercept, 3), round(R2, 3), hiper)
    names(parameters) <- c('Model', 'RMSLE', 'Slope', 'Intercept', 'R2', 'hp -> nu', 'hp -> gamma', 'hp -> cost', 'hp -> epsilon')
    rownames(parameters) <- ''
    write.csv(parameters, file = file.path(path, glue("Param_{Nombre}.csv")), row.names = FALSE)
  } else {
    RMSLE <- rmsle(conjunto$actual_DE, conjunto$predicciones_DE)
    R2 <- as.numeric((R$estimate)^2)
    parameters <- data.frame(Nombre, round(RMSLE, 3), round(slope, 3), round(intercept, 3), round(R2, 3))
  ...
  lollipop_plot <- ggplot(lollipop_data, aes(x = Valores_Reales, y = Errores)) +
    ...
    ggtitle(paste("Gráfico de Errores_", Nombre)) +
    xlab("Valores Reales") +
    ylab("Errores") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, size = 12)) +
  ...
  jpeg(file=file.path(path,glue("Reg_{Nombre}.jpg")), width = 500, height = 300)
    print(regression_plot_descaled)
  dev.off()
  
  # Save the plots as a JPG
  jpeg(file=file.path(path,glue("Lol_{Nombre}.jpg")), width = 500, height = 450)
      print(lollipop_plot)
  dev.off()
